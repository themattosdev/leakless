...
- [**Expectativas Customizadas do Pest**](./pest.md): `expect($class)->toBeLeakless()`, `expect($closure)->toRunCleanly()` e `expect($target)->toResetContainerState()` para testes automatizados.
